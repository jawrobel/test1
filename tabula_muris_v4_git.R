...
p <- DimPlot(pbmc, reduction = "tsne", label = TRUE, group.by = "cell_type_number", pt.size = 0.5, label.size = 10) +
...
pdf(paste(sample_name, gene_type, "DotPlot_Kat-cell-type_fig_lung.pdf", sep = "_"), width = 12, height = 8)
...
p <- DotPlot(pbmc, features = found_genes_paper, group.by = "Kat_cell_type_plot", scale = TRUE) + coord_flip() +
  ...
  # theme(axis.text.x = element_text(angle = 90, hjust = 0.95, vjust = 0.5),
        theme(axis.text.x = element_text(size = 27, angle = 45, hjust = 1),
        axis.text.y = element_text(size = 27, face = "italic"),
        axis.title = element_blank(),     
        plot.title = element_text(size = 34, face = "plain", hjust = 0.5, vjust = 0),
      axis.ticks.length = unit(0.075, "inch"),
        # axis.title = element_text(size = 16, face = "plain"),
        # axis.text = element_text(size = 16)
...
for (current_annotation_type in annotation_type) {
  ...
  # Violin Plots are only made for Lung (Figure 2)
  ...
  for (current_gene in found_genes) {
    ...
    p <- VlnPlot(object = pbmc, features = current_gene, group.by = "Kat_cell_type_plot", pt.size = 1, raster = TRUE) + 
      ...
      plot.title = element_text(size = 23, face = "italic"),
      axis.ticks.length = unit(0.1, "inch"),
